const { makeWASocket, useMultiFileAuthState, Browsers, getContentType,
   ...
   class ClusterWhatsAppSessionManager extends EventEmitter {
       ...
       async createInstanceWithPairingCode(phone, config = {}) {
           ...
           return await this.createInstance(phone, pairingConfig);
       }
       ...
       async checkInstanceExistsGlobally(phone) {
           try {
               const response = await MAIN.clusterproxy.sendWithAck('cluster-find-instance', { phone, clusterId: F.id }, { retries: 2, timeout: 1000 });
   ...
   class ClusterProxy {
       constructor(managerHub, config = {}) {
           ...
           this.connections = MAIN.connections = new Map();
               ...
               this.listenWithAck('cluster-find-instance', async (payload) => {
           const { phone } = payload;
           const instance = MAIN.instances.get(phone);
           return { found: !!instance, clusterId: F.id };
       });
   
       this.listenWithAck('cluster-schema-call', async (payload) => {
           const { schema, action, data, params, query } = payload;
           if (!schema || !action) throw new Error('Invalid schema/action');
   
           return new Promise((resolve, reject) => {
               NEWACTION(schema, action, {
                   params: params,
                   query: query,
                   body: data,
                   callback: resolve,
       ...
       this.listenWithAck('proxy-ws-message', async (payload) => {
   ...
   ON('ready', function () {
       ...
       hub.on('ready', function (managers) {
           console.log('ManagerHUB ready: ', F.id);
           let index = Object.keys(managers)[0];
           MAIN.sessionManager = managers[index];
